...
if __name__ == '__main__':
    ...
    if (now.month == 1 and now.day == 1 and now.hour == 0 and now.minute == 0 and now.second == 0):
        ListCodigo = array_codigos_estaciones ()
        for i in ListCodigo:
            dataOneYear(i)
    estacion_three_year = []
    fecha_ini_year = dt.replace(year=dt.year-4)
    fecha_fin_year = dt.replace(year=dt.year-3)
    estacion_three_year = get_estacion_by_date(codigo_estacion, fecha_inicial, fecha_final)
    mongo estacion_three_year export.js > out.csv
    app.run()
